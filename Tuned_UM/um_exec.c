...
/*************************************************************************PROTOTYPES*/
static inline Codeword get_codeword(uint32_t word, Codeword codeword);
static inline void interp_word(Codeword word, unsigned *registers, Memseg_T program, unsigned * idx);
...
extern void Interp_prog(Memseg_T program, unsigned *registers){
    ...
    while(1){
        word = (((program->array)[0])->array)[ctr];
        //word = Memseg_loads(program,0,ctr);
...
static inline Codeword get_codeword(uint32_t word, Codeword codeword){
    ...
    unsigned opcode = word >> 28;

    //Determine if we have a 3 or 2 register instruction and then assemble the
    //designated codeword
    if (opcode == 13){
        codeword.opcode = opcode;
        codeword.a = (word << 4) >> 29;
        codeword.b = (word << 7) >> 7;
    }
    else{
        codeword.opcode = opcode;
        codeword.a = (word << 23) >> 29;
        codeword.b = (word << 26) >> 29;
        codeword.c = (word << 29) >> 29;
    }
